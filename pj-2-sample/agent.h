	...
	virtual void close_episode(const std::string& flag = "") {
        ...
        for (int i=0 ; i < int(episode.size()) ; ++i){
            ...
            for (int j_num_tuple=0 ; j_num_tuple < _num_tuple ; ++j_num_tuple ){  
                weights[ _tuple_set[j_num_tuple] ][ tuple_to_idx(episode[i].before, j_num_tuple) ]  = weights[ _tuple_set[j_num_tuple] ][ tuple_to_idx(episode[i].before, j_num_tuple) ] + 0.0025 * ( update_score ) ;               
            } 
        }
	}
